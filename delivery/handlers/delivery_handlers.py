...
async def change_order(callback: types.CallbackQuery, state: FSMContext, sql_con:sql_con, bot:Bot):
    ...
    message_to_del = await func.send_photo(chat_id=callback.from_user.id, bot = bot, photo=data['path_image'], caption = callback.message.text.replace("НОВЫЙ ЗАКАЗ\n", "Вы взяли новый заказ. Если вы не заберете заказ в течении 12 часов, он автоматически отменится.\n"), reply_markup=kb.delivery_private_keyboard)
    sql_con.modify_order(order_id, 'delivery_private_messageid', message_to_del.message_id)
